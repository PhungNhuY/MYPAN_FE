...
const collection: ICollection = await getCollectionById(collectionId);
console.log(collection);

const category = ref<String>(collection.category as string)
const imageCoverLink = ref<String | null>(collection.imageCoverLink as string);
const name = ref<String>(collection.name as string);
const des = ref<String>(collection.description as string);
const postIds = ref<String[]>(collection.posts?.map(i => i._id) as string[]);
...
async function update() {
    ...
    const ans = await UpdateCollection(collectionId, data);
...
<template>
    <div class="container wrapper">
        <h2 class="head-title">Cập nhật bộ sưu tập</h2>

...
const imageCoverLink = ref<string | null | undefined>(post.imageCoverLink);
const name = ref<string | undefined>(post.name);
const des = ref<string | null | undefined>(post.description);
const ration = ref<number | null | undefined>(post.ration);
const time = ref<number | null | undefined>(post.time);
const ingres = ref<IIngre[]>(post.ingredients as IIngre[]);
const steps = ref<IStep[]>(post.steps as IStep[]);
...
const schema = Yup.object().shape({
    name: Yup.string().required('Trường này không được để trống').max(1000, 'Tối đa 1000 ký tự'),
    des: Yup.string().max(1000, 'Tối đa 1000 ký tự'),
    ration: Yup
        .number()
        .nullable()
        .min(1, 'Tối thiểu 1 khẩu phần ăn')
        .max(10, 'Tối đa 10 khẩu phần ăn')
    ...
    time: Yup
        ...
        .max(43200, 'Thời gian tối đa là 43200 phút')
        .transform((value) => (isNaN(value) || value === undefined) ? null : value)
    ,
});
</script>
...
<template>
    <Form @submit="create" :validation-schema="schema" v-slot="{ errors, isSubmitting }">
        <div class="container wrapper">
            <h2 class="head-title">Cập nhật món ăn</h2>
            <MyCard class="base-info">
                ...
                <div class="row">
                    <div class="col-6">
                        ...
                        <Field name="name" type="text" class="base-input form-control"
                            :class="{ 'is-invalid': errors.name }" v-model="name" />
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <Field name="ration" type="number" class="base-input form-control"
                                    :class="{ 'is-invalid': errors.ration }" v-model="ration" />
                                <div class="invalid-feedback">{{ errors.ration }}</div>
                            </div>
                            <div class="col-6">
                                <p class="time pt-2">Thời gian nấu (phút)</p>
                                <!-- <input class="base-input" type="number" placeholder="30 phút" min="1" v-model="time"> -->
                                <Field name="time" type="number" class="base-input form-control"
                                    :class="{ 'is-invalid': errors.time }" v-model="time" />
                    ...
                    <div class="col-6">
                        ...
                        <Field name="des" as="textarea" class="base-textarea description-content"
                            :class="{ 'is-invalid': errors.des }" rows="3" v-model="des" />
